...
impl Layer {
    ...
    pub fn pixels(&self) -> &Vec<Vec<u32>> { &self.pixels }
...
impl Image {
    ...
    fn generate_final_layer(&self) -> Layer {
        ...
        for layer in self.layers() {
            for (cur_pixel, final_pixel) in layer.pixels().iter().flatten().zip(final_layer_pixels.iter_mut())  {
                match &final_pixel {
                    0 | 1 => (),
...
fn num_ones_times_num_twos(layer: &Layer) -> usize {
    ...
    layer
        ...
        .iter()
        .flatten()
        .for_each(|p| match p { 1 => ones += 1, 2 => twos += 1, _ => (), } );
