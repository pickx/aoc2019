...
fn parse_input(input: &str) -> (Vec<Steps>, Vec<Steps>) {

    let input_lines: Vec<&str> = input.lines().collect();
    assert_eq!(input_lines.len(), 2);
    let (first_path_str, second_path_str): (&str, &str) = (input_lines[0], input_lines[1]);
...
pub fn part1() -> usize {
    ...
    let (first_path, second_path) = parse_input(INPUT_2019_3);
